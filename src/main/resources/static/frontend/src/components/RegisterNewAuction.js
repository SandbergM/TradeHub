...
const RegisterNewAuction = () => {
  ...
  const removeImage = (file, index) => {
    if (index === primaryPickIndex) {
      setPrimaryPickIndex(0);
    }
    if (index < primaryPickIndex) {
      setPrimaryPickIndex(primaryPickIndex - 1);
    }
  ...
  const displayDay = (days) => {
    ...
    return `${days} Dagar - ${timestampToDay(
      date
    )} ${date.getDate()} ${timestampToMonth(
      date
    )} kl ${timestampToHourAndMinutes(date)}`;
  };
  ...
  return (
    <Form id="register-auction-form" onSubmit={submitAuction}>
      <div className="col-12 light-grey-background pt-4 pb-4 pl-3 pr-3 bold tradeHub-dark-grey">
        ...
        <Label
          className="register-auction-label mt-4 pl-2"
          for="register-auction-description"
        >
          Beskrivning
        </Label>
        <textarea
        ...
        <Input
          ...
          className="col-12 register-auction-input pl-2"
          onChange={(e) => {
            setAuction({ ...auction, timestamp: e.target.value });
          }}
        >
          {auctionDurationInterval.map((value, index) => {
            return (
              <option key={index} value={timestampConverter(value)}>
                {" "}
                {displayDay(value)}{" "}
              </option>
            );
        ...
        {images.length !== 0 && (
          <div className="row ">
            ...
            {images.map((val, index) => {
              return (
                <div
                  className="col-6 col-md-6 col-lg-6 register-auction-image-item mt-2 justify-content-center"
                  key={index}
                >
                  <ImagePreview
                    image={val}
